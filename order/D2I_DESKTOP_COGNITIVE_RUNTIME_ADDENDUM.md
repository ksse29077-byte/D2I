...
Episodic Memory and Case Learning Records` are complete. The platform now has
...
OFFICE-300 adds a Desktop-owned spreadsheet dispatcher, bounded XLSX package
worker, deterministic query/context-slice boundary, and a hidden pinned Excel
COM worker for typed formulas and recalculation. Models receive typed facts,
not workbook bytes, XML, ranges, formula text, paths, or COM. Excel mutation is
...
raw XML/XPath/COM. Role, Case, lease, Work Grant, workspace, backend approval,
Policy, one-shot activation, exact artifact generation, worker/application
hash, and fresh post-save reopen are mandatory. Word is macro-disabled and WFP
loopback-only for live Completion. Legacy HWP remains license-gated.
...
stop 계약으로 연결된다. `D2I-KRN-500 - First Complete Verified Single-Task
E2E`까지 완료되어 Track K는 닫혔다. 현재 first active task는
`D2I-WORK-100 - Role Contract v1`이며 Work Radar와 Case Queue는 아직
시작하지 않는다.
