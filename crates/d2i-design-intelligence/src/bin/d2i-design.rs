use d2i_design_intelligence::{
    parse_design_json_strict, DesignWorkCertificationV1, DesignWorkCompletionReportV1,
...
fn main() {
    ...
    let result = match arguments.as_slice() {
        [kind, action, flag, path]
            if action == "verify" && flag == "--input" && kind == "completion" =>
        {
            verify_completion(path)
        }
        [kind, action, flag, path]
            if action == "verify" && flag == "--input" && kind == "replay" =>
        {
            verify_replay(path)
        }
        ...
        _ => Err(
            "usage: d2i-design completion verify --input <file> | replay verify --input <file> | certification verify --input <file> --public-key <hex-file>".to_owned(),
        ),
    };
...
fn verify_certification(path: &str, key_path: &str) -> Result<(), String> {
    ...
    let certification: DesignWorkCertificationV1 =
        parse_design_json_strict(&bytes).map_err(|error| error.to_string())?;
...
fn verify_completion(path: &str) -> Result<(), String> {
    let bytes = std::fs::read(path).map_err(|error| error.to_string())?;
    let report: DesignWorkCompletionReportV1 =
        parse_design_json_strict(&bytes).map_err(|error| error.to_string())?;
...
fn verify_replay(path: &str) -> Result<(), String> {
    let bytes = std::fs::read(path).map_err(|error| error.to_string())?;
    let report: DesignWorkReplayReportV1 =
        parse_design_json_strict(&bytes).map_err(|error| error.to_string())?;
    report.validate_gate().map_err(|error| error.to_string())
}
