...
const COGNITIVE_SCHEMA_VERSION: u32 = 1;
...
impl ObservationSnapshot {
    ...
    pub fn compute_state_hash(&self) -> Result<String, DesktopError> {
...
impl PlanNode {
    fn validate(&self) -> Result<(), DesktopError> {
        ...
        match (self.kind, self.loop_bound) {
            ...
            (PlanNodeKind::LoopBounded, _) => Err(DesktopError::Invalid(format!(
                "loop_bounded node '{}' must declare loop_bound 1..={MAX_PLAN_LOOP_ITERATIONS}",
                self.node_id
            ))),
            (_, Some(_)) => Err(DesktopError::Invalid(format!(
...
fn validate_version(version: u32, label: &str) -> Result<(), DesktopError> {
