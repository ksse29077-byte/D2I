...
$modulePath = Join-Path $PSScriptRoot 'lib/WorkforceCheckpoint.psm1'
...
function Hash-Text([string]$Text) {
    return Get-WorkforceSha256Text $Text
}
...
try {
    ...
    Invoke-Test 'forced-step-3-failure-resumes-after-step-2' {
        ...
        $oneCheckpoint = Read-WorkforceCheckpoint $one
        $two = New-TestCheckpoint 'step-2' 2 $base $artifact2 $bindings $true @($oneCheckpoint.checkpoint_sha256)
        Assert-True (Test-WorkforceCheckpoint $one $base (Hash-Text 'input-step-1') $output).Valid 'step 1 was not reusable'
        Assert-True (Test-WorkforceCheckpoint $two $base (Hash-Text 'input-step-2') $output @($oneCheckpoint.checkpoint_sha256)).Valid 'step 2 was not reusable'
    ...
    Invoke-Test 'artifact-tamper-invalidates-checkpoint' {
        ...
        Assert-True (-not (Test-WorkforceCheckpoint $path $base (Hash-Text 'input-artifact-tamper') $output).Valid) 'tampered artifact was accepted'
    }
    ...
    Invoke-Test 'runner-hash-invalidates-downstream' {
        ...
        Assert-True (-not (Test-WorkforceCheckpoint $path $changed (Hash-Text 'input-runner-binding') $output).Valid) 'runner mutation was accepted'
    }
    ...
    Invoke-Test 'model-hash-invalidates-model-backed-downstream' {
        ...
        Assert-True (-not (Test-WorkforceCheckpoint $path $changed (Hash-Text 'input-model-binding') $output).Valid) 'model mutation was accepted'
    }
    ...
    Invoke-Test 'cohort-change-invalidates-holdout' {
        ...
        Assert-True (-not (Test-WorkforceCheckpoint $path $changed (Hash-Text 'input-cohort-binding') $output).Valid) 'cohort mutation was accepted'
    }
    ...
    Invoke-Test 'readiness-only-change-reuses-raw-session' {
        ...
        Assert-True (Test-WorkforceCheckpoint $path $changed (Hash-Text 'input-raw-session') $output).Valid 'readiness-only change invalidated raw session evidence'
    }
    ...
    Invoke-Test 'cleanup-failure-never-checkpoints-success' {
        ...
        Assert-True (-not (Test-WorkforceCheckpoint $path $base (Hash-Text 'input-cleanup-failure') $output).Valid) 'cleanup failure was reusable'
    }
    ...
    Invoke-Test 'failure-diagnostic-precedes-cleanup-and-redacts-secret' {
        ...
        $diagnostic = Write-WorkforceFailureDiagnostic `
    ...
    Invoke-Test 'resume-finished-hash-verifies' {
        ...
        $finished.finished_sha256 = Get-WorkforceObjectHash $payload
        Assert-True ($finished.finished_sha256 -eq (Get-WorkforceObjectHash $payload)) 'resumed finished hash differs'
    }
    ...
    Invoke-Test 'fresh-and-resumed-normalized-semantics-match' {
        ...
        Assert-True ((Get-WorkforceObjectHash $fresh) -eq (Get-WorkforceObjectHash $resumed)) 'normalized semantic result differs'
    }
    ...
    Invoke-Test 'checkpoint-and-diagnostic-contain-no-sensitive-data' {
        ...
        try {
            ...
            Write-WorkforceCheckpoint -Path (Join-Path $checkpointRoot 'sensitive.json') -Checkpoint $checkpoint
        }
        catch { $rejected = $true }
