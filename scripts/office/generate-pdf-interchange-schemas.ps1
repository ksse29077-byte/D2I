...
$contractPath = Join-Path $repoRoot 'crates/d2i-pdf-interchange/src/contracts.rs'
$schemaRoot = Join-Path $repoRoot 'schemas/pdf'
$generatedRoot = Join-Path $repoRoot 'target/pdf-schema-generation'
...
foreach ($name in $structs.Keys) {
    ...
    $document = [ordered]@{
        ...
        '$id' = "https://schemas.d2i.local/pdf/$(Convert-Kebab $name)-v1.schema.json"
        title = $name
        '$ref' = "#/`$defs/$name"
...
if ($Check) {
    if (-not (Test-Path -LiteralPath $schemaRoot)) { throw 'PDF schema directory is absent.' }
    ...
    if (($expected.Name -join '|') -ne ($actual.Name -join '|')) { throw 'PDF schema file set drift detected.' }
    foreach ($file in $expected) {
        ...
        if ((Get-FileHash -Algorithm SHA256 -LiteralPath $file.FullName).Hash -ne (Get-FileHash -Algorithm SHA256 -LiteralPath $actualPath).Hash) {
            throw "PDF schema drift detected: $($file.Name)"
        }
    }
    Write-Output "PDF schemas match generator: $($expected.Count)"
    exit 0
}
...
Write-Output "Generated PDF Interchange schemas: $($structs.Count)"
