...
$contractPath = Join-Path $repoRoot 'crates/d2i-design-intelligence/src/contracts.rs'
$schemaRoot = Join-Path $repoRoot 'schemas/design'
$generatedRoot = Join-Path $repoRoot 'target/design-schema-generation'
...
function New-StringSchema([string]$Field) {
    ...
    return [ordered]@{ type = 'string'; minLength = 1; maxLength = 2048 }
}
...
function New-TypeSchema([string]$Type, [string]$Field) {
    if ($Type.StartsWith('Option<') -and $Type.EndsWith('>')) {
        return [ordered]@{
            oneOf = @(
                (New-TypeSchema $Type.Substring(7, $Type.Length - 8) $Field),
                [ordered]@{ type = 'null' }
            )
        }
    }
    if ($Type.StartsWith('Vec<') -and $Type.EndsWith('>')) {
        return [ordered]@{
            type = 'array'
            maxItems = 4096
            items = New-TypeSchema $Type.Substring(4, $Type.Length - 5) $Field
        }
    }
    switch ($Type) {
        ...
        'i32' { return [ordered]@{ type = 'integer'; minimum = -2147483648; maximum = 2147483647 } }
        default {
            if (-not $enums.Contains($Type) -and -not $structs.Contains($Type)) {
                throw "Unsupported schema type '$Type' on '$Field'"
            }
...
function Get-ReferencedType([string]$Type) {
    $current = $Type
    while (($current.StartsWith('Option<') -or $current.StartsWith('Vec<')) -and
        $current.EndsWith('>')) {
        if ($current.StartsWith('Option<')) {
            $current = $current.Substring(7, $current.Length - 8)
        }
        else {
            $current = $current.Substring(4, $current.Length - 5)
        }
    }
    return $current
}
...
foreach ($name in $structs.Keys) {
    ...
    while ($pending.Count -gt 0) {
        ...
        foreach ($field in $structs[$current].GetEnumerator()) {
            ...
            if (($structs.Contains($referenced) -or $enums.Contains($referenced)) -and
                -not $reachable.Contains($referenced)) {
    ...
    foreach ($definitionName in $definitions.Keys) {
        if ($reachable.Contains($definitionName)) {
            $reachableDefinitions[$definitionName] = $definitions[$definitionName]
        }
    }
    $document = [ordered]@{
        '$schema' = 'https://json-schema.org/draft/2020-12/schema'
        '$id' = "https://schemas.d2i.local/design/$(Convert-Kebab $name)-v1.schema.json"
        title = $name
        '$ref' = "#/`$defs/$name"
...
if ($Check) {
    if (-not (Test-Path -LiteralPath $schemaRoot)) { throw 'Design schema directory is absent.' }
    ...
    if (($expected.Name -join '|') -ne ($actual.Name -join '|')) { throw 'Design schema file set drift detected.' }
    foreach ($file in $expected) {
        ...
        if ((Get-FileHash -Algorithm SHA256 -LiteralPath $file.FullName).Hash -ne
            ...
            throw "Design schema drift detected: $($file.Name)"
        }
    }
    Write-Output "Design schemas match generator: $($expected.Count)"
    exit 0
}
...
Write-Output "Generated Design Intelligence schemas: $($structs.Count)"
